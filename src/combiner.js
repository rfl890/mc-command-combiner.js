...
const getCustomCommand = (id, command, type) => {
    switch (type) {
        case 1:
...
const getCompiledCommand = (version, commands, des) => {
    const lines = splitLines(commands);
    let customCommands = "";
    switch (version) {
        case "1.18.x - 1.14.x":
        ...
        case "1.8.x":
            ...
            for (let i = 0; i < 3 + lines.length + num; i++) {
                customCommands = customCommands + "}";
            }
            return customCommands;
    }
